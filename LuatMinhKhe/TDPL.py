...
def is_link_duplicate(link):
    if os.path.exists("TuDienPhapLuatMinhKhe.json"):
        with open("TuDienPhapLuatMinhKhe.json", "r", encoding="utf-8") as json_file:
    ...
    try:
        ...
        while retry_count < 20:
            ...
            for item_news in item_news_elements:
                ...
                with open("TuDienPhapLuatMinhKhe.json", "w", encoding="utf-8") as json_file:
...
# Khai báo URL gốc
base_url = "https://luatminhkhue.vn/tu-dien-phap-luat.aspx"

# Tạo danh sách URL cần duyệt
# urls = [f"{base_url}?page={page}" for page in range(1, 2600)]
urls = [f"{base_url}?page={page}" for page in range(1, 268)]
